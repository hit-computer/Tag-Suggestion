...
运行 `category.py` 文件对原始数据类别标签进行过滤，根据设定的类别数目（程序中的NofC值）筛选出重要的NofC个类别标签。由于筛选策略中会用到词向量，所以需要预先训练词向量（替换程序中的 `vec100.txt` 文件）

`preprocess.py` 文件将训练数据和测试数据处理成符合模型输入的格式。将数据按照实验设置划分成 `trainingdata.txt` 和 `testdata.txt` 两部分。运行 `preprocess.py` 会得到4个文件：`MT_WordEmb.pkl`、`ttrain.dialogues.pkl`、`tvalid.dialogues.pkl` 和 `ttrain.dict.pkl`，这四个文件分别是词向量，训练数据，测试数据和词表，用于设置 `state.py` 中的对应参数
